  ...
  return (
    <div className="space-y-6">
      ...
      <div className="border-b border-slate-200 pb-4">
        ...
        <p className="text-sm text-slate-600">Share your experience and learn from fitness professionals</p>
      </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            ...
            <input
              ...
              className="w-full border border-slate-200 bg-white px-4 py-2.5 text-sm text-slate-900 placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-[#7B9669] focus:border-[#7B9669]"
            />
          </div>
            ...
            <div className="flex gap-2">
              {[1, 2, 3, 4, 5].map((star) => (
                <button
                  ...
                  className={`text-2xl transition-all ${star <= feedback.rating ? 'text-[#7B9669]' : 'text-slate-300'} hover:text-[#8aa678]`}
                >
                  ★
                </button>
              ))}
          ...
          <div>
            ...
            <textarea
              ...
              className="w-full border border-slate-200 bg-white px-4 py-2.5 text-sm text-slate-900 placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-[#7B9669] focus:border-[#7B9669] resize-none"
            />
          </div>
          ...
          <button
            type="submit"
            disabled={submitting}
            className="w-full bg-[#7B9669] hover:bg-[#8aa678] text-white font-bold py-3 text-sm transition-all disabled:opacity-50 disabled:cursor-not-allowed"
          >
            {submitting ? 'Submitting...' : 'Submit Feedback'}
          </button>

          {submitMessage && (
            <div className="border border-[#d4e0ce] bg-[#f0f5ed] px-4 py-2.5 text-xs text-[#7B9669] text-center">
              {submitMessage}
            </div>
          )}
        </form>
      ...
      <div className="bg-white p-6 space-y-4">
        ...
        <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-2">
          {expertSuggestions.map((item) => (
            <div
              key={item.id}
              className="bg-[#f0f5ed] p-5 rounded-lg hover:shadow-md transition-shadow text-center"
            >
              <div className="flex justify-center mb-4">
                <img
                  src={item.avatar}
                  alt={item.expert}
                  className="h-16 w-16 rounded-full object-cover border-3 border-[#d4e0ce]"
                />
              </div>
              <h3 className="font-bold text-slate-900 text-sm mb-1">{item.expert}</h3>
              ...
              <p className="text-sm text-slate-700 leading-relaxed mb-3">{item.suggestion}</p>
              <span className="inline-block text-xs font-semibold px-3 py-1 rounded-full bg-[#e8f0e3] text-[#7B9669]">
                {item.category}
              </span>
            </div>
          ))}
